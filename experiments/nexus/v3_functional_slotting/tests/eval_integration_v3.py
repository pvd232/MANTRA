...
def run_eval_v3():
    ...
    nexus.load_state_dict(torch.load(exp_dir / "checkpoints/model_v3_alpha08.pt", map_location=device))
    nexus.eval()
    
    # 3. Memory Warming Pass
    print("Warming Nexus memory with training data...", flush=True)
    ...
    print(f"\n--- V3 Performance Summary (Functional Slotting) ---")
    print(f"Alpha:         {cfg.alpha}")
    print(f"Baseline MSE:  {avg_mse_b:.6f}")
    print(f"Nexus V3 MSE:  {avg_mse_n:.6f}")
    ...
    with open(exp_dir / "audits/eval_results.json", "w") as f:
