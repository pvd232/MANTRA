...
class CentroidAddressableManifold(nn.Module):
    def __init__(
        ...
        # Class-based Bucketing Buffer
        self.register_buffer("class_lookup", torch.arange(vocab_size, dtype=torch.long))
    ...
    def _load_class_map(self, path: str):
        # Path is expected to be the .npy file for now
        if path.endswith(".json"):
             # Backwards compatibility or alternative
             pass
        elif path.endswith(".npy"):
            data = np.load(path)
            data_t = torch.from_numpy(data).long()
            # regulators start at 0
            n_reg = data_t.shape[0]
            self.class_lookup[:n_reg].copy_(data_t)
            # For non-regulators, they remain mapped to their own IDs (default arange)
            print(f"Loaded class lookup for {n_reg} regulators from {path}")
    ...
    def read(self, query_emb, slot_values, slot_keys, tids=None, alpha=0.5):
        ...
        # [V3] Functional Bucketing
        buckets = self.class_lookup[lex_ids] % self.n_buckets
        ...
        if tids is not None and alpha < 1.0:
...
class NexusV3(nn.Module):
    def __init__(self, vocab_size, hidden_size, num_layers=4, n_buckets=512, slots_per_bucket=32, persistent_slots=31, class_map_path=None, **kwargs):
        super().__init__()
        self.vocab_size = vocab_size
        self.hidden_size = hidden_size
        ...
        self.alpha_val = kwargs.get("alpha", 0.8) # [V3] Increased Semantic Reliance
        ...
        self.cam = CentroidAddressableManifold(hidden_size, vocab_size, n_buckets, slots_per_bucket, persistent_slots, class_map_path=class_map_path)
